...
public class ConsultationEventTest {
	...
	public void testExecute() {
		...
		if (!boxRoom.getState().equalsIgnoreCase("full") || !shockRoom.getState().equalsIgnoreCase("full"))
			fail("Rooms have wrong state");
		if (!phys1.getState().equalsIgnoreCase("visiting") || !phys2.getState().equalsIgnoreCase("visiting"))
			fail("Physicians not busy");
